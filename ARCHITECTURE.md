...
旧仕様では、共通語や `matched_keywords` の一致を使って既存 topic に結合する結合型ロジックを採用していた。
現在は、決定的な `topic_key` による分割型ロジックへ変更している。
...
load_config → setup_logging → fetch → normalize → summarize(optional) → classify/score → deduplicate(optional) → assign_topics → summarize_topics → build_daily_report → build_action_suggestions → split → markdown → save
