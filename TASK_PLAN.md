...
目的  
・英語版Markdownを保持したまま、日本語で読める別ファイルを追加保存できるようにする

完了状態  
・`src/translators/markdown_translator.py` を追加済み  
・`collected_articles_ja.md` / `monitored_articles_ja.md` を追加保存可能  
・履歴保存有効時に `*_ja_YYYY-MM-DD.md` も保存可能  
・英語版保存仕様を維持  
...
目的  
・日本語版Markdownの翻訳品質を改善する

完了状態  
・ローカルOllama優先の翻訳を追加済み  
・接続先は `http://localhost:11434/api/generate` である  
・Ollama失敗時は辞書翻訳または原文で継続する  
・保存仕様と既存pipeline構造を維持  
